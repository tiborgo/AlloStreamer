...
{
	gameThread = boost::thread(boost::bind(&CubemapPreviewWindow::gameLoop, this));
}

void CubemapPreviewWindow::gameLoop()
{
	// Init GLFW
	glfwInit();
	...
	glfwSetKeyCallback(window, CubemapPreviewWindow::key_callback);
	glfwSetCursorPosCallback(window, CubemapPreviewWindow::mouse_callback);
	glfwSetScrollCallback(window, CubemapPreviewWindow::scroll_callback);
	...
	shader = new Shader(STR(AlloPlayer_RESOURCE_DIR) "cubemaps.vs",
		STR(AlloPlayer_RESOURCE_DIR) "cubemaps.frag");

	skyboxShader = new Shader(STR(AlloPlayer_RESOURCE_DIR) "skybox.vs",
		STR(AlloPlayer_RESOURCE_DIR) "skybox.frag");
...
// +Z (front)? (CHECK THIS)
// -Z (back)?
GLuint CubemapPreviewWindow::loadCubemap(std::vector<const GLchar*> faces)
{
    GLuint textureID;
...
GLuint CubemapPreviewWindow::loadTexture(GLchar* path)
{
...
void CubemapPreviewWindow::Do_Movement()
{
    // Camera controls
    if (keys[GLFW_KEY_W])
...
void CubemapPreviewWindow::key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
...
void CubemapPreviewWindow::mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    if (firstMouse)
    {
        lastX = xpos;
