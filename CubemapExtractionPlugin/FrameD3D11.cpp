...
FrameD3D11::FrameD3D11(boost::uint32_t width,
	...
	resource(resource)
{
	cudaError_t error = cudaGraphicsD3D11RegisterResource(&cudaResource, cpuTexturePtr, cudaGraphicsRegisterFlagsSurfaceLoadStore);
	getLastCudaError("cudaGraphicsD3D11RegisterResource (g_texture_2d) failed");

	error = cudaMalloc(&cudaLinearMemory, (width * height * 3) / 2);
...
FrameD3D11::~FrameD3D11()
{
	...
	error = cudaFree(cudaLinearMemory);
